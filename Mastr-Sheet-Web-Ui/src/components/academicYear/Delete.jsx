...
import { deleteAcademicYear} from "../../redux/academicYear/deleteAcademicYear";
...
export default function Delete({ item }) {
  ...
  const handleSubmit = async () => {
    if (!form.validate().hasErrors) {
      try {
        await dispatch(deleteAcademicYear(item.id)); 
        handleClose(); // Close modal on success
        // Handle success further (e.g., clear form, show notification)
      } catch (error) {
        // Handle errors appropriately
        console.error("Error delete AcademicYear:", error);
      }
    }
  };
  ...
  return (
    <>
      ...
      <Modal opened={isOpen} onClose={handleClose} title="حذف السنة الدراسية" centered>
